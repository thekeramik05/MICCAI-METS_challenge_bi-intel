#!/usr/bin/env python3
"""Simple(50/50) vs Weighted 앙상블, 클래스별 FP confidence 히스토그램 + 임계값별 제거 개수."""
        ...
        for lab in LABELS:
            ...
            if n_pred > 0:
                ...
                for i in range(n_pred):
                    if poverlap[i] == 0:
                        fp_conf[lab].append(float(pred_conf[i]))
        out[name] = fp_conf
    return out


def main():
    agg = {name: {lab: [] for lab in LABELS} for name in ["Simple5050", "Weighted"]}
    ...
    for f in FOLDS:
        ...
        with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
            for i, res in enumerate(ex.map(process_case, args), 1):
                for name in agg:
                    for lab in LABELS:
                        agg[name][lab].extend(res[name][lab])
                if i % 100 == 0 or i == len(cases):
                    print(f"  fold_{f}: {i}/{len(cases)} done", flush=True)

    for lab in LABELS:
        print(f"\n===== {LABEL_NAMES[lab]}: FP confidence 히스토그램 + 임계값별 제거 =====")
        for name in ["Simple5050", "Weighted"]:
            vals = np.array(agg[name][lab])
            n = len(vals)
            print(f"\n-- {name} (총 FP={n}) --")
            if n == 0:
                continue
            ...
            for c, lo, hi in zip(counts, edges[:-1], edges[1:]):
                bar = "#" * int(round(c / max(counts.max(), 1) * 30))
                print(f"  [{lo:.1f},{hi:.1f}): {c:4d} {bar}")
            print(f"  mean={vals.mean():.3f} median={np.median(vals):.3f}")

            print(f"  -- 임계값 T 미만 confidence FP를 지운다면 --")
            for t in THRESHOLDS:
                removed = int((vals < t).sum())
                print(f"    T={t}: 제거 {removed}/{n} ({removed/n:.1%}), 잔존 {n-removed}개")
